...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        allPosts.addAll(Arrays.asList(
                ...
                new Post("The Fine Art of Receiving Pets", lorumIpsumGenerator(5), dog),
                new Post("ON NAPPING", lorumIpsumGenerator(5), dog),
                new Post("Paw Full of Treats", lorumIpsumGenerator(5), dog)
        ));

        User cat = userService.findByUsername("cat");
        allPosts.addAll(Arrays.asList(
                new Post("Cataphract: Stratagems For Human Mounted Combat", lorumIpsumGenerator(5), cat),
                new Post("Yarn is Great!", lorumIpsumGenerator(5), cat),
                new Post("No Human, No Cry", lorumIpsumGenerator(5), cat),
                new Post("10 Easy Tricks to Prevent Your Human From Sleeping", lorumIpsumGenerator(5), cat),
                new Post("The Black Cat Strikes Back", lorumIpsumGenerator(5), cat),
                new Post("Being Productive While Sleeping 20 Hours per Day", lorumIpsumGenerator(5), cat)
        ));

        User otter = userService.findByUsername("otter");
        allPosts.addAll(Arrays.asList(
                new Post("History of the Otterman Empire", lorumIpsumGenerator(5), otter),
                ...
                new Post("Dam Nation", lorumIpsumGenerator(5), otter),
                new Post("An Ode to My Significant Otter", lorumIpsumGenerator(5), otter),
                new Post("To Live and Die in the L.A. River", lorumIpsumGenerator(5), otter)
        ));
        ...
        allPosts.addAll(Arrays.asList(
                ...
                new Post("Clawligula", lorumIpsumGenerator(5), sloth),
                new Post("Fur Algae", lorumIpsumGenerator(5), sloth)
        ));
    ...
    private String lorumIpsumGenerator(int num) {
        ...
        for (int i = 0; i <= numParagraphs; ++i) {
            lorumIpsum.append('\t');
            lorumIpsum.append(texts.get(rand.nextInt(texts.size())));
            lorumIpsum.append('\n');
        }
